...
the shipped script still speaks the `Stop` hook stdin/stdout protocol.

This body is shipped BYTE-IDENTICALLY by both Drivers (livespec-driver-claude
at `.claude-plugin/hooks/`, livespec-driver-codex at `livespec/hooks/`); this
test pins its contract.

Contract under test:

- WARN (a `{"systemMessage": ...}` payload on stdout, exit 0): the last turn
  wrote a PLANNING ARTIFACT — a `*handoff*.md`, or any `.md` under a `plan/`
  or `prompts/` directory — whose written content carries at least
  `CHECKBOX_THRESHOLD` (3) markdown checkbox task items (`- [ ]` / `- [x]`).
- SILENT (empty stdout, exit 0): a planning artifact below the checkbox
  threshold; checkbox-looking syntax that is inline prose rather than a real
  list item; a write to a NON-planning path even with many checkboxes.
- FAIL-OPEN / SILENT (empty stdout, exit 0): malformed (non-JSON) stdin;
  `stop_hook_active` true; a transcript whose path does not exist.
"""
