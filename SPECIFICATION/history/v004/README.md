# livespec-driver-claude — specification

This tree is the natural-language specification for `livespec-driver-claude`,
the reference **Claude Code Driver** for the livespec family: the thin,
agent-runtime-specific SKILL.md bindings (plus a small plugin-shipped
hook bundle and a structural gate) through which a human drives the
livespec spec lifecycle interactively. The Driver dogfoods `livespec` —
...
- `contracts.md` — the Driver-owned seam contracts: plugin/marketplace
  manifest shape, the eight-skill binding set, the core-root resolution
  algorithm, the fenced-invocation discipline, the hook-bundle surface,
  and versioning.
...
the slash-command surface, the wrapper-CLI shapes, the schemas, and the
hook *disciplines/postures* are all core-owned; this tree governs only
the Driver-local realization of that seam. The `compat.pinned` value in
the repo-root `.livespec.jsonc`'s `livespec-driver-claude` section records
