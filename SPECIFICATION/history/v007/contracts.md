...
`contracts.md` §"Install verification", which owns the rule that a project
...
- **records present, but none whose `projectPath` is the project root** —
  the defective state core's §"Install verification" requires be "detected
...
§"Install verification" already records the mechanism, noting that
