...
for example `drive`; the row renders as
`drive (livespec-orchestrator-beads-fabro)` with kind `Skill`. The
colon-qualified form `livespec-orchestrator-beads-fabro:drive` is still
...
2. If the change will modify tracked files, create a dedicated worktree
   ...
   worktree lives under the per-user root
   `~/.worktrees/livespec-driver-claude/<branch>`. Create it with the
   ...
   A raw `mise exec -- git -C /data/projects/livespec-driver-claude worktree add -b <branch>
   "$HOME/.worktrees/livespec-driver-claude/<branch>" master` also yields a usable
...
scale set `livespec-driver-claude-k3s` (livespec-s43svm.16's per-repo
